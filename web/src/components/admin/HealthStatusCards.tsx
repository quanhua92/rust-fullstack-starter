  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            Uptime: {basicHealthQuery.data?.data?.uptime ? formatUptime(basicHealthQuery.data.data.uptime) : 'Unknown'}
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center space-x-2 mb-2">
            {getStatusIcon((livenessQuery.data?.data as any)?.status || 'unknown')}
            <div className="text-2xl font-bold capitalize">
              {(livenessQuery.data?.data as any)?.status || 'Unknown'}
            </div>
          </div>
          {getStatusBadge((livenessQuery.data?.data as any)?.status || 'unknown')}
          <p className="text-xs text-muted-foreground mt-2">
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center space-x-2 mb-2">
            {getStatusIcon((readinessQuery.data?.data as any)?.status || 'unknown')}
            <div className="text-2xl font-bold capitalize">
              {(readinessQuery.data?.data as any)?.status || 'Unknown'}
            </div>
          </div>
          {getStatusBadge((readinessQuery.data?.data as any)?.status || 'unknown')}
          <p className="text-xs text-muted-foreground mt-2">
      ...
      <Card className="md:col-span-2 lg:col-span-4">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Overall system health score: {getOverallHealthScore()}% of dependencies are healthy
              </CardDescription>
            </div>
            <Link 
              to="/admin/health" 
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-lg font-semibold text-green-600">
                {detailedHealthQuery.data?.data?.checks ? 
                  Object.values(detailedHealthQuery.data.data.checks).filter((c: ComponentHealth) => c.status === 'healthy').length : 0}
              </div>
            ...
            <div className="text-center">
              <div className="text-lg font-semibold text-yellow-600">
                {detailedHealthQuery.data?.data?.checks ? 
                  Object.values(detailedHealthQuery.data.data.checks).filter((c: ComponentHealth) => c.status === 'degraded').length : 0}
              </div>
            ...
            <div className="text-center">
              <div className="text-lg font-semibold text-red-600">
                {detailedHealthQuery.data?.data?.checks ? 
                  Object.values(detailedHealthQuery.data.data.checks).filter((c: ComponentHealth) => c.status === 'unhealthy').length : 0}
              </div>
          ...
          <div className="mt-4 text-xs text-muted-foreground">
            Last updated: {detailedHealthQuery.data?.data?.timestamp ? 
              new Date(detailedHealthQuery.data.data.timestamp).toLocaleString() : 
              'Unknown'}
          </div>
        </CardContent>
      </Card>
