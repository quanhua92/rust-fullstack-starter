...
export function UserActivityAnalytics() {
  ...
  const engagementMetrics = useMemo(() => {
    ...
    const activeUsers = userActivityData[userActivityData.length - 1]?.active_users || 0
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{engagementMetrics.dailyActiveUsers}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Tasks per User</CardTitle>
            <Activity className="h-4 w-4 text-green-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{engagementMetrics.tasksPerUser}</div>
            <p className="text-xs text-muted-foreground mt-1">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">{engagementMetrics.completionRate}%</div>
            <p className="text-xs text-muted-foreground mt-1">
              Task success rate
            </p>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{engagementMetrics.avgSessionLength}</div>
            <p className="text-xs text-muted-foreground mt-1">
              Session duration
            </p>
          </CardContent>
              ...
              <BarChart data={hourlyActivityData}>
                ...
                <Bar dataKey="activity" fill="#8884d8">
                  {hourlyActivityData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.peak ? '#10B981' : '#94A3B8'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-4">
                  <Avatar className="h-12 w-12">
                    <AvatarFallback>
                      {currentUserQuery.data.data.username.substring(0, 2).toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <div className="font-semibold">{currentUserQuery.data.data.username}</div>
                    <div className="text-sm text-muted-foreground">{currentUserQuery.data.data.email}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Badge className="mt-1">{currentUserQuery.data.data.role}</Badge>
                  </div>
                  <div>
                    <div className="text-sm font-medium">Status</div>
                    <Badge variant="outline" className="mt-1 text-green-600">
                      Active {/* Note: is_active not available in current API response */}
                    </Badge>
                  </div>
                  <div>
                    <div className="text-sm font-medium">Email Verified</div>
                    <Badge variant="outline" className="mt-1">
                      Yes {/* Note: email_verified not available in current API response */}
                    </Badge>
                  </div>
                  <div>
                    <div className="text-sm font-medium">Last Login</div>
                    <div className="text-sm text-muted-foreground mt-1">
                      Never {/* Note: last_login_at not available in current API response */}
                    </div>
                  </div>
                </div>

                <div>
                  <div className="text-sm font-medium mb-2">Account Created</div>
                  <div className="text-sm text-muted-foreground">
                    Recently {/* Note: created_at not available in current API response */}
                  </div>
                </div>
              </div>
            ) : (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 bg-blue-50 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600">
                    {userActivityData[userActivityData.length - 1]?.login_attempts || 0}
                  </div>
                  <div className="text-sm text-muted-foreground">Login Attempts Today</div>
                </div>
                <div className="text-center p-3 bg-green-50 rounded-lg">
                  <div className="text-2xl font-bold text-green-600">
                    {userActivityData[userActivityData.length - 1]?.task_creation || 0}
                  </div>
                  <div className="text-sm text-muted-foreground">Tasks Created Today</div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <Badge variant="outline">{engagementMetrics.tasksPerUser}</Badge>
                </div>
              </div>
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center space-x-2 text-sm text-muted-foreground">
            ...
            <span>User activity data refreshes automatically every 15-30 seconds. Last updated: {new Date().toLocaleString()}</span>
          </div>
        </CardContent>
      </Card>
    </div>
