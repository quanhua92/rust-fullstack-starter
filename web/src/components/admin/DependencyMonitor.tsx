					...
					<CardContent>
						{detailedHealthQuery.isLoading ? (
							...
							<Alert>
								...
								<AlertDescription>
									Failed to load core dependencies:{" "}
				...
				<Card>
					...
					<CardContent>
						{readinessQuery.isLoading ? (
							...
							<div className="space-y-4">
								{(() => {
									const readinessChecks = getProbeChecks(
										readinessQuery.data?.data,
									);
									return (
										readinessChecks &&
										Object.entries(readinessChecks).map(
											([name, check]: [string, ComponentHealth]) => (
												<div
													key={`readiness-${name}`}
													className="flex items-start justify-between p-4 border rounded-lg"
				...
				<CardContent>
					{startupQuery.isLoading ? (
						...
						<Alert>
							<AlertTriangle className="h-4 w-4" />
							<AlertDescription>
								Failed to load startup dependencies:{" "}
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							{(() => {
								const startupChecks = getProbeChecks(startupQuery.data?.data);
								return (
									startupChecks &&
									Object.entries(startupChecks).map(
										([name, check]: [string, ComponentHealth]) => (
											<div
												key={`startup-${name}`}
												className="flex items-start justify-between p-4 border rounded-lg"
				...
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
						...
						<div className="flex items-center justify-between p-3 bg-gray-50 rounded">
							...
							<span className="text-muted-foreground">
								{(() => {
									...
									return isProbeResponse(readinessData) &&
						...
						<div className="flex items-center justify-between p-3 bg-gray-50 rounded">
							...
							<span className="text-muted-foreground">
								{(() => {
									const startupData = startupQuery.data?.data;
									return isProbeResponse(startupData) && startupData.timestamp
