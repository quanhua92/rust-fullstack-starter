	...
	useEffect(() => {
		...
		if (prevTaskStats && currentStats) {
			...
			if (currentStats.completed > prevTaskStats.completed) {
				...
				if (newCompletions >= 5) {
					// Only notify for significant completions
					addNotification({
						type: "task",
						severity: "success",
		...
		if (
			prevHealthStatus &&
			currentHealth &&
			prevHealthStatus !== currentHealth
		) {
				...
				<CardContent>
					...
					<div className="mt-6 pt-4 border-t text-xs text-muted-foreground">
						<div className="flex items-center space-x-2">
							...
							<span>
								Real-time notifications are active. Data updates automatically
