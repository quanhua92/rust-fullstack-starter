...
mod tests {
    ...
    async fn test_create_and_get___MODULE_NAME__() {
        ...
        assert_eq!(created.priority, retrieved.priority);
    }

    #[tokio::test]
    async fn test_list___MODULE_NAME_PLURAL___with_filters() {
        let database = create_test_database().await;
        ...
        for i in 1..=5 {
            let request = Create__MODULE_STRUCT__Request {
                ...
                status: if i % 2 == 0 { Some(__MODULE_STRUCT__Status::Active) } else { Some(__MODULE_STRUCT__Status::Inactive) },
                priority: Some(i),
        ...
        let response = list___MODULE_NAME_PLURAL___service(&database, list_request).await.unwrap();
        assert_eq!(response.items.len(), 2); // Even numbered items
        ...
        let response = list___MODULE_NAME_PLURAL___service(&database, list_request).await.unwrap();
        assert_eq!(response.items.len(), 2); // Items with priority 3 and 4
    }
    ...
    async fn test_bulk_operations() {
        ...
        let bulk_response = bulk_create___MODULE_NAME_PLURAL___service(&database, bulk_create_request).await.unwrap();
        assert_eq!(bulk_response.success_count, 2);
        assert_eq!(bulk_response.error_count, 0);

        // Test bulk delete
        let ids: Vec<Uuid> = bulk_response.results.iter().map(|item| item.id).collect();
        let bulk_delete_request = Bulk__MODULE_STRUCT__DeleteRequest {
            ids,
            skip_errors: Some(false),
        };

        let delete_response = bulk_delete___MODULE_NAME_PLURAL___service(&database, bulk_delete_request).await.unwrap();
        assert_eq!(delete_response.success_count, 2);
        assert_eq!(delete_response.error_count, 0);
    }

    #[test]
    fn test_cursor_generation_and_parsing() {
        let offset = 100;
        let cursor = generate_cursor(offset);
        let parsed_offset = parse_cursor(&cursor).unwrap();
        assert_eq!(offset, parsed_offset);
    }
}