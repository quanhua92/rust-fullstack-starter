...
) -> Result<Json<ApiResponse<Vec<__MODULE_STRUCT__>>>, ApiError> {
    // Check permissions
...
pub async fn get___MODULE_NAME__(
    ...
    RequireAuth(auth_user): RequireAuth,
    Path(id): Path<Uuid>,
) -> Result<Json<ApiResponse<__MODULE_STRUCT__>>, ApiError> {
    // Check permissions
    rbac_services::require_user_or_higher(&auth_user)?;
