...
async fn test_get_user_by_id() {
    ...
    let (_auth_user, token) = factory.create_authenticated_user(&format!("auth_{}", &unique_username)).await;
    let response = app.get_auth(&format!("/users/{}", user.id), &token.token).await;
...
async fn test_update_user_profile() {
    ...
    let response = app.put_json_auth("/auth/me", &update_data, &token.token).await;
    
    assert_status(&response, StatusCode::OK);
    let json: serde_json::Value = response.json().await.unwrap();
    assert_json_field(&json["user"]["email"], "", &serde_json::Value::String("newemail@example.com".to_string()));
}
...
async fn test_get_nonexistent_user() {
    ...
    let response = app.get_auth(&format!("/users/{}", fake_id), &token.token).await;
