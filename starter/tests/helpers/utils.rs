...
pub fn assert_json_field(json: &Value, field: &str, expected: &Value) {
    ...
    assert_eq!(actual, expected, "Expected field '{}' to be {}, got {}", field, expected, actual);
}

/// Assert JSON field exists
pub fn assert_json_field_exists(json: &Value, field: &str) {
    assert!(!json[field].is_null(), "Expected field '{}' to exist", field);
}

/// Extract error message from response
pub async fn extract_error_message(response: reqwest::Response) -> String {
    let json: Value = response.json().await.expect("Failed to parse error response");
    json["error"]["message"]
        .as_str()
