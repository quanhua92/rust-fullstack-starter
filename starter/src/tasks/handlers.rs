...
use crate::tasks::types::{TaskContext, TaskResult, TaskError};
...
impl TaskHandler for EmailTaskHandler {
    async fn handle(&self, context: TaskContext) -> Result<TaskResult, TaskError> {
        ...
        let to = context.payload.get("to")
            .and_then(|v| v.as_str())
            .ok_or_else(|| TaskError::Execution("Missing 'to' field in email payload".to_string()))?;
        
        let subject = context.payload.get("subject")
            .and_then(|v| v.as_str())
            .ok_or_else(|| TaskError::Execution("Missing 'subject' field in email payload".to_string()))?;
        
        let body = context.payload.get("body")
            .and_then(|v| v.as_str())
            .ok_or_else(|| TaskError::Execution("Missing 'body' field in email payload".to_string()))?;
        ...
        if body.contains("fail") {
            return Err(TaskError::Execution("Email service temporarily unavailable".to_string()));
        }
...
impl TaskHandler for DataProcessingTaskHandler {
    async fn handle(&self, context: TaskContext) -> Result<TaskResult, TaskError> {
        let data = context.payload.get("data")
            .ok_or_else(|| TaskError::Execution("Missing 'data' field in payload".to_string()))?;
        
        let operation = context.payload.get("operation")
            .and_then(|v| v.as_str())
            .unwrap_or("process");
        ...
        let result = match operation {
            "count" => {
                if let Some(array) = data.as_array() {
                    ...
                    return Err(TaskError::Execution("Data is not an array for count operation".to_string()));
                }
            }
            "sum" => {
                if let Some(array) = data.as_array() {
                    ...
                    return Err(TaskError::Execution("Data is not an array for sum operation".to_string()));
                }
            }
            "process" => {
            ...
            _ => {
                return Err(TaskError::Execution(format!("Unknown operation: {}", operation)));
            }
        };
...
impl TaskHandler for FileCleanupTaskHandler {
    async fn handle(&self, context: TaskContext) -> Result<TaskResult, TaskError> {
        let file_path = context.payload.get("file_path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| TaskError::Execution("Missing 'file_path' field in payload".to_string()))?;
        
        let max_age_hours = context.payload.get("max_age_hours")
            .and_then(|v| v.as_u64())
            .unwrap_or(24);
        
        tracing::info!("Cleaning up files in path: {}, max age: {} hours", file_path, max_age_hours);
...
impl TaskHandler for ReportGenerationTaskHandler {
    async fn handle(&self, context: TaskContext) -> Result<TaskResult, TaskError> {
        let report_type = context.payload.get("report_type")
            .and_then(|v| v.as_str())
            .ok_or_else(|| TaskError::Execution("Missing 'report_type' field in payload".to_string()))?;
        
        let start_date = context.payload.get("start_date")
            .and_then(|v| v.as_str())
            .ok_or_else(|| TaskError::Execution("Missing 'start_date' field in payload".to_string()))?;
        
        let end_date = context.payload.get("end_date")
            .and_then(|v| v.as_str())
            .ok_or_else(|| TaskError::Execution("Missing 'end_date' field in payload".to_string()))?;
        
        tracing::info!("Generating {} report from {} to {}", report_type, start_date, end_date);
        
        // Simulate report generation (this could be quite long for real reports)
        tokio::time::sleep(std::time::Duration::from_secs(2)).await;
        
        let report_url = format!("/reports/{}/{}-{}-{}.pdf", 
            report_type, report_type, start_date, end_date);
...
impl TaskHandler for WebhookTaskHandler {
    async fn handle(&self, context: TaskContext) -> Result<TaskResult, TaskError> {
        let url = context.payload.get("url")
            .and_then(|v| v.as_str())
            .ok_or_else(|| TaskError::Execution("Missing 'url' field in payload".to_string()))?;
        
        let _payload = context.payload.get("payload")
            .cloned()
            .unwrap_or_else(|| serde_json::json!({}));
        
        let method = context.payload.get("method")
            .and_then(|v| v.as_str())
            .unwrap_or("POST");
        ...
        if url.contains("fail") {
            return Err(TaskError::Execution("Webhook endpoint returned 500".to_string()));
        }
